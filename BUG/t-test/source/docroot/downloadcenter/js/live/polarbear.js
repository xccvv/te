 ...
 (function($) {
	var Addons = function(element, options) {
		...
		this.offerOptionalAddon = function(id) {			
			...
			if ($("#chrchk").length) {
				if($.browser.msie && parseFloat($.browser.version) >= 7.0 && parseFloat($.browser.version) < 9.0){
					var inputcheckbox3 = $("<input/>").attr('type','checkbox').attr('id','chrExtCheckbox').attr("checked", true).css({position: 'relative'});				
				} else {
					var inputcheckbox3 = $("<input/>").attr('type','checkbox').attr('id','chrExtCheckbox').attr("checked", true);
				}
				inputcheckbox3.on("change",obj.chrExtChkboxEventHandler);
				var spanforimagebase3 = $("<span/>").attr('class','CheckboxImage');
				var spanforimage3 = (IE_7_UserAgent) ? spanforimagebase3 : spanforimagebase3.attr('id','CheckboxImage3');
				var labelforcheckbox3 = $("<label/>").attr('for','chrExtCheckbox');
				labelforcheckbox3.append(spanforimage3);			
				labelforcheckbox3.append($("#chrchkText"));	
				var checkbox3 = $("<div/>").attr('id','AUTO_ID_js_div_offer_checkbox3').addClass('Checkbox');
				var learnMoreChrExt = $("<p id=\"AUTO_ID_js_p_more_links\" style=\"padding-left :25px;\"/>").append("<a rel=\"learnmorechrext\" href=\"#\" id=\"AUTO_ID_js_a_learnmore\">"+ $("#chrchkLearn").text() +"</a>");
				var marketingChromeExt = $("<div id=\"learnMorePanelChromeExt\" style=\"display: none;margin-top:-10px;\"/>").append($("<p />").append($("#chrchkLearnTxt").text()));
			...
			if(addon == undefined){
				...
				if(addon.abbr === "dual"){	
					var learnMoreChr = $("<p id=\"AUTO_ID_js_p_more_links\" style=\"padding-left :25px;\"/>").append("<a rel=\"learnmorechr\" href=\"#\" id=\"AUTO_ID_js_a_learnmore\">"+ addon.text.learnmore +"</a>");
					...
					var learnMoreMss = $("<p id=\"AUTO_ID_js_p_more_links\" style=\"padding-left :25px;\"/>").append("<a rel=\"learnmoremss\" href=\"#\" id=\"AUTO_ID_js_a_learnmore\">"+ addon.text.learnmore +"</a>");
					var learnMoreMsc = $("<p id=\"AUTO_ID_js_p_more_links\" style=\"padding-left :25px;\"/>").append("<a rel=\"learnmoremsc\" href=\"#\" id=\"AUTO_ID_js_a_learnmore\">"+ addon.text.learnmore +"</a>");
				...
				if(addon.abbr === "dual") {
					...
					var container = $("<div id=\"AUTO_ID_js_div_offer_container\" />").append(checkbox).append(checkbox2).append(learnMoreChr).append(checkbox1).append(learnMoreGtb).append(image).append(marketingGTB).append(marketingChrome);										
				}
				else if(addon.abbr === "mdual") {					
					var marketingMss = $("<div id=\"learnMorePanelMss\" style=\"display: none;\" />").append($("<hr>")).append($("<p />").append(addon.text.marketingMss));
					var marketingMsc = $("<div id=\"learnMorePanelMsc\" style=\"display: none;\" />").append($("<hr>")).append($("<p />").append(addon.text.marketingMsc));
					var container = $("<div id=\"AUTO_ID_js_div_offer_container\" />").append(checkbox).append(learnMoreMss).append(checkbox1).append(learnMoreMsc).append(image).append(marketingMss).append(marketingMsc);
				}
				else if (addon.abbr === "gtb" || addon.abbr === "chr"){
					var container = $("<div id=\"AUTO_ID_js_div_offer_container\" />").append(checkbox).append(checkbox2).append(checkbox1).append(learnMore).append(image).append(marketing);	
				}
				else if(addon.abbr === "msc") {
					var container = $("<div id=\"AUTO_ID_js_div_offer_container\" />").append(checkbox).append(learnMore).append(image).append(marketing);
				}
				else {
